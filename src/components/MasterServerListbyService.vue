    ...
    <v-app>
     ...
     <v-card class="mt-5">
      <v-card-title>
        ...
        <v-select :items="servicenames_data" label="Service Filter" v-model="serviceFilter" solo></v-select>
        <v-spacer></v-spacer> 
        <v-text-field
          v-model="searchInput"
          append-icon="search"
        ...
        data() {
                return {
                    ...
                    exportedheaders: [{
                        ...
                        text: 'TCC Service Name',
                        value: ['TCC Service', 'tccservicename']
                    }, {
                        text: 'Active',
                        value: ['Active', 'active']
                    }, {
            ...
            created() {
                    axios
                    .get(process.env.API_URL + '/getservicenames') // Call the get function passing the url of the API as a parameter
                    .then(response => (this.servicenames_data = response.data)) // Setting masterserverlist_data to the data gotten from the API 
                    .catch(function(err) {
            ...
            computed: {
                ...
                let serviceFilter = this.serviceFilter
                return this.masterserverlist_data.filter(function(server) {
                if (serviceFilter !== '') {
                    return unfiltered || (server.active === 'Y' && server.tccservicename === serviceFilter)
                }
                else  return unfiltered || (server.active === 'Y')
                })
                },
                arraytoheaders() {
